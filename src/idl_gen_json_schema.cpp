...
static std::string GenArrayType(const Type &type) {
  ...
  if (type.struct_def != nullptr) {
    ...
    // Handle union elements properly - generate anyOf schema for union tables
  ...
  bool generate() {
    ...
    for (auto e = parser_.enums_.vec.cbegin(); e != parser_.enums_.vec.cend();
      ...
      if ((*e)->is_union) {
        ...
        // Handle regular enums - generate string enum
    ...
    for (auto s = parser_.structs_.vec.cbegin();
      ...
      for (auto prop = properties.cbegin(); prop != properties.cend(); ++prop) {
        ...
        if (property->value.type.base_type == BASE_TYPE_UNION) {
          // This is a union value field, generate conditions for each variant
